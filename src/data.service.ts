...
export class DataService {
  ...
  async findProductsByCategory(idCategory: number): Promise<Product[]> {
    const products = await this.productsRepository.findBy({ category : idCategory })
    return products;
  ...
  async findOneTableByPosition(x: number, y: number): Promise<Table | null> {
    const tables = await this.tablesRepository.findOneBy({ posX : x, posY : y})
    return tables;
  }
  ...
  async seedProducts(): Promise<void> {
    ...
    const Products = this.loadDefaultProducts();
  ...
  async seedTables(): Promise<void> {
    ...
    const Tables = this.loadDefaultTables();
